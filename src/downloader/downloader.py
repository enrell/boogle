...
def _download_book(
    book_id: str, output_dir: Path, log_file: Path, pre_meta: dict | None = None
) -> tuple[str, Path | None, dict, str | None]:
...
class BookSeeder:
    ...
    def _process_metadata_only(self, batch_meta: list[dict]) -> int:
        """Process metadata-only batch in light mode (no file downloads)."""
        count = 0
    ...
    def seed_all(self, limit: int | None = None, batch_size: int = 500) -> int:
        # Light mode uses different checkpoint file
        if self.light_mode:
            checkpoint_file = self.output_dir / ".checkpoint_light"
        else:
        ...
        for meta in self.iter_all_books(limit=limit):
            ...
            if len(batch) >= batch_size:
                ...
                if self.light_mode:
                    # Light mode: just upsert metadata, no downloads
                    count = self._process_metadata_only(filtered_batch)
                    total += count
                    downloaded_ids.update(m["book_id"] for m in filtered_batch)
                else:
                    # Full mode: download files
                ...
                skipped_ids = {m["book_id"] for m in batch} - {
                    m["book_id"] for m in filtered_batch
                }
                downloaded_ids.update(skipped_ids)

                checkpoint_file.write_text("\n".join(downloaded_ids))
                print(
                    f"Seeded {total} books (skipped {len(skipped_ids)} super-documents)"
                )
                batch = []
    ...
    def _process_batch(
        self, batch_meta: list[dict]
    ) -> list[tuple[str, Path | None, dict, str | None]]:
        results = []
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(
                    _download_book, m["book_id"], self.output_dir, self.log_file, m
                ): m
                for m in batch_meta
            }
            for future in as_completed(futures):
