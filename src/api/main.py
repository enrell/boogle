...
async def lifespan(app: FastAPI):
    ...
    if use_light_mode:
        # Light mode: use metadata-only index
...
async def search_books(query: str, limit: int = 10):
    ...
    if use_light_mode:
        # Light mode: use metadata-only search
        if metadata_indexer is None:
            raise HTTPException(
                status_code=500, detail="Metadata indexer not initialized"
            )
        ...
        # Build results list
        results = []
        for book_id, score in raw_results[:limit]:
        ...
        if realtime_indexer is None:
            raise HTTPException(
                status_code=500, detail="Realtime indexer not initialized"
            )
    ...
    for book_id, base_score, chunk_id in raw_results:
        ...
        title_norm = " ".join(
            "".join(c for c in title.lower() if c.isalnum() or c.isspace()).split()
        )
        author_norm = " ".join(
            "".join(c for c in author.lower() if c.isalnum() or c.isspace()).split()
        )
    ...
    # Sort & Format
    sorted_unique = sorted(unique_books.values(), key=lambda x: x[0], reverse=True)[
        :limit
    ]
...
async def add_document(request: AddDocumentRequest):
    ...
    metadata = json.dumps(
        {"book_id": request.book_id, "title": request.title, "author": request.author}
    )

    doc_id = realtime_indexer.add_document(request.content, metadata)

    return AddDocumentResponse(
        doc_id=doc_id, message=f"Document added with ID {doc_id}"
    )
...
async def flush_documents():
    ...
    return FlushResponse(
        flushed_count=count, message=f"Flushed {count} documents from memory"
    )
